...
class Name(Expr):
    ...
    def eval(self, env):
        ...
        raise TypeError("ill-defined arithmetic errors of {} operator".format(self.var_name))
...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        return self.operator.eval(env).apply(values)
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        parapairs = zip(self.parameters, arguments)
        for parapair in parapairs:
            new_env[parapair[0]] = parapair[1]

        #values = [argument.eval(new_env) for argument in arguments]
