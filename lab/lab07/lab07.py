def insert_into_all(item, nested_list):
    ...
    """
    a = []
    for list_each in nested_list:
        a.append(list_each.copy())
    for list_a in a:
...
def trade(first, second):
    ...
    len1 = len(first)
    len2 = len(second)
    equal_prefix = lambda: m <= len1 and n <= len2
    while sum(first[:m]) != sum(second[:n]) and m <= len1 and n <= len2:
...
def reverse(lst):
    ...
    for i in range(len_ // 2):
        lst[i], lst[len_ - 1 - i] = lst[len_ - 1 - i], lst[i]
...
def make_glookup(class_assignments):
    ...
    """
    curtotal, curget = 0, 0
    def current_grade(dic, marks):
        nonlocal curtotal, curget
        curtotal, curget = curtotal + cs61a[dic], curget + marks
        return curget / curtotal
