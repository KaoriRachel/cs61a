...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            ...
            possibilities.append((skel_char + result, cost))
        # Insert
        result, cost = helper_align(skeleton_idx, code_idx + 1)
        possibilities.append(("+[" + code_char + "]" + result, cost + 1))
        # Delete
        result, cost = helper_align(skeleton_idx + 1, code_idx)
        possibilities.append(("-[" + skel_char + "]" + result, cost + 1))
...
def num_splits(s, d):
    ...
    def num_equal(s, d):
        if len(s) == 1:
            if abs(s[0]) == d:
                ...
                return 0
        return num_equal(s[1:], abs(d - s[0])) + num_equal(s[1:], d + s[0])
    ...
    if len(s) == 1:
        if abs(s[0]) <= d:
            ...
            return 0
    if s[0] <= d:
        return (num_splits(s[1:], d - s[0]) + num_splits(s[1:], d + s[0]))
    return num_splits(s[1:], d + s[0]) - num_splits(s[1:], s[0] - d) + num_equal(s[1:], s[0] - d) 
