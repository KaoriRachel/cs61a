...
def distance(city_a, city_b):
    ...
    5.0
    """
    lona, lata = get_lon(city_a),get_lat(city_a);
    lonb, latb = get_lon(city_b),get_lat(city_b);
    return sqrt((lona - lonb) ** 2 + (lata - latb) ** 2)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    'Bucharest'
    """
    ran = make_city('ran', lat, lon)
    disa, disb = distance(ran, city_a), distance(ran, city_b)
    if disa < disb:
