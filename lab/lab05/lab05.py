...
def riffle(deck):
    ...
    for i in range(length // 2):
        total = total + [deck[i], deck[i + length // 2]]
    return total
    #return [deck[i] deck[i + length // 2] for i in range(length // 2)]
...
def add_trees(t1, t2):
    ...
    branchCouple = zip(branches(t1), branches(t2))
    mergeBranches = [add_trees(couple[0], couple[1]) for couple in branchCouple]
    len1, len2 = len(branches(t1)), len(branches(t2))
    if len1 > len2:
        for i in range(len2,len1):
            mergeBranches.append(branches(t1)[i])
    elif len1 < len2:
        for i in range(len1,len2):
            mergeBranches.append(branches(t2)[i])
    return tree(label_new, mergeBranches)
