...
def balanced(m):
    ...
    l, r = end(arm_l), end(arm_r)
    turque_left = total_weight(l) * length(arm_l)
    turque_right = total_weight(r) * length(arm_r)
    return turque_left == turque_right and balanced(end(left(m))) and balanced(end(right(m)))
...
def mul_interval(x, y):
    ...
    value in y."""
    ux, uy = upper_bound(x), upper_bound(y);
    lx, ly = lower_bound(x), lower_bound(y);
    p1 = ux * uy
    p2 = ux * ly
    p3 = uy * lx
    p4 = lx * ly
...
def quadratic(x, a, b, c):
    ...
    if (mid <= low and a > 0) or (mid >= up and a < 0):
        return interval(f(low), f(up))
    elif (mid <= low and a < 0) or (mid >= up and a > 0):
        return interval(f(up), f(low))
    elif a > 0:
        return interval(f(mid), max(f(low),f(up)))
    else:
        return interval(min(f(low),f(up)), f(mid))
