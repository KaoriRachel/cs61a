...
def make_withdraw(balance, password):
    ...
    """
    error_password = []
    def with_draw(amount, pass_word):
        nonlocal balance
        nonlocal error_password
        if len(error_password) == 3:
            return "Too many incorrect attempts. Attempts: ['" + error_password[0] + "', '" + error_password[1] +"', '" + error_password[2] + "']"
            #return "Too many incorrect attempts. Attempts: " + error_password
        if pass_word == password:
            if balance < amount:
                return 'Insufficient funds'
            balance = balance - amount
            return balance
        error_password.append(pass_word)
...
def repeated(t, k):
    ...
    lastItem = 0
    for item in t:
        if continue_num == 0 or item == lastItem:
...
def remainders_generator(m):
    ...
    11
    """
    def inner_generator(divide_num):
        for num in naturals():
            if num % m == divide_num:
                yield num
    for i in range(m):
