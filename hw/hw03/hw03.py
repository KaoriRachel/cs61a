...
def g_iter(n):
    ...
    True
    """
    top , mid , end , now = 1 , 2 , 3 , 3
    if n <= now :
        return n
    else :
        while now != n :
            top , mid , end = mid , end , end + 2 * mid + 3 * top 
            now = now + 1
    return end
...
def count_change(total):
    ...
    True
    """
    def count_change_max(total , max):
        if total == 0 :
            return 1
        elif total < 0 :
            return 0
            ...
            return count_change_max(total , max // 2) + count_change_max(total - max , max)
    index = log(total) / log(2)
    index = index // 1
    return count_change_max(total , 2 ** index)
...
def make_anonymous_factorial():
    ...
    True
    """
    return (lambda fac: lambda n: fac(fac,n))(lambda fac,n: 1 if n == 1 else mul(n, fac(fac,sub(n, 1))))
