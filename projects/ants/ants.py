...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while target_place != beehive:
            ...
            elif i > self.max_range:
                return None
            else:
                target = rANTdom_else_none(target_place.bees)
                if target:
                    return target
                else:
                    target_place = target_place.entrance
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        # bees_copy =
        # self.place.bees[0].armor -= 3
        # print(bees_copy == self.place.bees)
        ...
        for bee in self.place.bees[:]:  # bees_copy
            bee.reduce_armor(0)
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, gamestate):
        ...
        ptr_place = self.place
        while ptr_place.exit is not None:
            ptr_place = ptr_place.exit
            cur_ant = ptr_place.ant
            if cur_ant is not None:
                if cur_ant not in self.buff_ants:
                    self.buff_ants.append(ptr_place.ant)
                    cur_ant.damage *= 2
                if isinstance(cur_ant, ContainerAnt):
                    in_ant = cur_ant.contained_ant
                    if in_ant is not None and in_ant not in self.buff_ants:
                        self.buff_ants.append(in_ant)
                        in_ant.damage *= 2
        # END Problem 13
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        final_action = self.inner_action
        self.is_ahead = True
        ...
        if gamestate.time % 2 == 0:
            for status in self.statuses:
                if status[1] > 0:
                    status[1] -= 1
                    final_action = status[0](final_action, self)
        else:
            for status in self.statuses:
                if status[1] > 0:
                    status[1] -= 1
                    final_action = status[0](final_action, self)
                    if status[0] == make_scare:
                        continue
                    else:
                        break
        final_action(gamestate)

    def inner_action(self, gamestate):
...
def apply_status(status, bee, length):
    ...
    # BEGIN Problem EC
    bee.statuses.insert(0, [status, length])
    # END Problem EC
...
class LaserAnt(ThrowerAnt):
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem OPTIONAL
        this_damage = self.damage - self.insects_shot * 0.05 - distance * 0.2

        return this_damage if this_damage > 0 else 0
        # END Problem OPTIONAL
